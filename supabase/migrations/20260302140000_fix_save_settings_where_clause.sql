-- Fix save_admin_settings to include WHERE clauses for singleton updates
-- This resolves the "UPDATE requires a WHERE clause" error
...
BEGIN
  ...
  SET
    ...
    updated_at = now()
  WHERE TRUE; -- Explicit WHERE clause for singleton update

  -- Update Notification Settings (Singleton)
  ...
  -- Update SMTP Settings (Singleton)
