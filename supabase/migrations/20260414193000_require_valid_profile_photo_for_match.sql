CREATE OR REPLACE FUNCTION public.has_valid_match_photo(p_avatar_url TEXT)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_avatar_url IS NULL THEN FALSE
    ELSE (
      btrim(lower(p_avatar_url)) NOT IN ('', 'null', 'undefined')
      AND POSITION('ui-avatars.com' IN lower(p_avatar_url)) = 0
      AND POSITION('api.dicebear.com' IN lower(p_avatar_url)) = 0
      AND (
        btrim(lower(p_avatar_url)) ~ '^https?://'
        OR btrim(p_avatar_url) ~ '^[^[:space:]]+/[^[:space:]]+$'
      )
    )
  END;
$$;
...
BEGIN
  ...
  WHERE l.event_id = p_event_id
    ...
    AND public.has_valid_match_photo(sender.avatar_url)
    AND NOT EXISTS (
      SELECT 1
      FROM public.match_passes pass_row
