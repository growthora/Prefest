-- Reset and rebuild RLS policies for core domain tables to avoid legacy policy conflicts.
-- Access model:
-- - Organizer: full control over own events and related rows.
-- - Buyer: full control over own rows.
-- - Admin: full control over all rows.
...
-- 3) Drop all existing non-system policies on target tables (legacy cleanup)
DO $$
DECLARE
...
-- 4) Recreate policies with canonical names
