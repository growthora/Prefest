-- Financial aggregate fix: enforce strict organizer/admin separation and avoid duplicated sums.
...
BEGIN
  ...
  RETURN (
    ...
    payment_financials AS (
      ...
      LEFT JOIN LATERAL (
        SELECT COALESCE(SUM(ps.value), 0) AS organizer_split
        FROM public.payment_splits ps
        WHERE ps.payment_id = p.id
          AND COALESCE(ps.recipient_type, 'organizer') = 'organizer'
    ...
    totals AS (
      SELECT
        ...
        COALESCE(SUM((GREATEST(gross_value - organizer_value, 0)) * 0.10), 0)::numeric AS platform_profit
